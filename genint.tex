...
\subsection{Implications of generics as a vague quantifier}
...
Instead, we present a view where generics are a kind of vague quantifier, which have the defining features of being vague, sensitive to a comparison class, and which quantify over subjective probabilities (cf., \citeNP{Sterken2015, Nickel2016} for related ideas about vagueness and context-sensitivities). 
...
Formally, this context-sensitivity is typically modeled as considering a comparison class of typically other objects of the same type \cite<e.g., an expensive hamburger is one that has a price that is high relative to other hamburgers;>{Kennedy2007}.
...
How does a listener know that the comparison class should be \emph{other animals}, and not, say, \emph{other objects} or \emph{other entities}?
The problem of determining an appropriate comparison class is a general problem shared with other context-sensitive language, including gradable adjectives and vague quantifers like \emph{many} or \emph{a lot} \cite{Kennedy2007, scholler2017semantic}
Recently, it has been shown that pragmatic reasoning, formalized in a similar model of language understanding, can be used to flexibly adjust the comparison class for interpreting gradable adjectives depending on the context \cite{Tessler2017, tessler2022warm}.
Future research should examine whether the same processes underly comparison class inferences when interperting generic language.
...
Though in these experiments, we did not try to discern the nature of these probabilities, in earlier work, we showed how human truth judgments are sensitive participants subjective, predictive probabilities and not the particular ground-truth statistics concerning prevalence \cite{Tessler2019psychrev}. 
This view can help explain why certain generics sound odd or false, even though the prevalence within the appropriate comparison class seems to suggest they should be true. 
A canonical example is \emph{Primary school teachers are female}, which is a statement that conventionally is considered false (or is rejected) even though most primary school teachers are in fact female (and more primary school teachers are female in comparison to other sets of teachers, or even many other professions). 
If a speaker believes that in the future, the probability that a primary school teacher will be female is close to 0.5 (i.e., there is no systematic gender bias in hiring), then the statement statement would be considered false or infelicitious under our model, similar to how it treats \emph{Birds are female} (cf., long-run frequencies in \citeNP{Cohen1999}'s theory of generics).
This fact that generics convey subjective probabilities may be the source of normative inferences from generics \cite{gockeritz2014young, roberts2017group, orvell2018s}: For example, it may be a fine line between an inference that the speaker believes the future probability is high with the inference that the speaker believes the future probability should be high.
...
Those probabilities are sampled from a prior disribution over prevalence, which we have conceived as being formed by knowledge of other categories within a comparison class.
...
For example, taxonomic category knowledge that enables inductive generalizations across levels of a heirarchy can be learned given a grammar-based prior over category relations \cite{Kemp2008}. 
Such a probabilisitic generative model has both conceptual knowledge encoded through the structure of the model and statistical information encoded with probabilities that tie together different parts of the structure.
In such a model, a prevalence prior could be constructed via marginalization over categorical or structural variables. 
With this kind of richer represention of kinds and properties (e.g., \citeNP{kemp2012integrated}), it is possible that rich conceptual knowledge \cite<\emph{a la}{Gelman2003, Gelman2009}, not just prevalence, could be learned with our formal prevalence-based meaning for generics. 
...
Our computational model of generic language is compatiable with representations of categories in terms both an exemplar and a prototype model (CITE). A learner who infers some level of prevalence from a novel generic statement could assign that probability to all members of the category identically (a la an exemplar approach), or could assign it specifically to prototypical members of the category and alter the imputed probabilty for instances that deviate from the prototype. Future experiments could try to disentangle these two alternatives. Future theoretical work additionally could try to relate these perpsectives with theories of generics that are built from prototype theory directly (CITE). Additionally, both exemplar and prototype representations could be imagined to play a role in the construction of the prevalence prior. For example, when a listener constructs a prevalence prior by considering the comparison class of \emph{other animals}, their representation of the comparison class (and the quantiative details of the prevalence prior) will likely vary if it is represented by exemplars of the superordinate category or by prototypical instances of other animals. %In our experimental work, we elicit the comparison class by asking participants to list other animals, and this distribution over animals is non-uniform  
...
\subseciton{Pragmatic reasoning and generic language}
