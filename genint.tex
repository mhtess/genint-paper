...
For example, gradable adjectives like \emph{tall} or \emph{expensive} do not convey a single precise quantity of height or price (e.g., an expensive watch lies at a different price-point than an expensive hamburger), and yet we do not draw the conclusion that the core meaning of \emph{tall} has nothing to do with height or that the meaning of \emph{expensive} is not really about the price of the object.
The meanings of gradable adjectives are often arrived at by considering a comparison class of other objects of the same type \cite<e.g., an expensive hamburger is one that has a price that is high relative to other hamburgers;>{Kennedy2007} or which is pragmatically felicitous \cite{Tessler2017, tessler2022warm}, and the meaning of vague adjectives can be modeled using an uncertain threshold-in a way completely analogous to how we have modeled the meaning of generics \cite{Lassiter2015}.
\mht{where should we have a statement about how pragmatics maybe could determine the comparison class for generics?}
...
\mht{Sensitivity to comparison class}
\mht{Comparison class can be inferred by pragmatics}
\mht{Internal, subjective probabilities}
