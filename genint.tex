...
Participants who completed Expt.~1a were barred from participating in Expt.~1b.
...
At the level of property type, \citeA{Cimpian2010} described a difference between the implied prevalence of generics about intrinsic properties (biological parts described with color adjectives; e.g., \emph{purple feathers}) and extrinsic, accidental properties (e.g., \emph{fungus-covered claws}).
...
Participants who completed Expt.~3a were barred from participating in Expt.~3b.
...
Statements like \emph{Lorches live in zoos}, \emph{Feps perform in the circus}, and \emph{Wugs have seizures} all received average implied prevalence ratings below 50\%, with many individual ratings of 10\% or less (Expt. 1b).
...
\subsection{Implications of vague quantifier view of generics}
...
This conclusion is often arrived at by authors implicitly considering only fixed-threshold models of a prevalence-based semantics.
Our formal implementation of models that assume fixed-thresholds support this narrow conclusion, that a fixed-threshold semantics for generics is difficult to support given the data.
...
Instead, we find strong evidence in support of a view of generics as a kind of vague quantifier, with the defining features of being vague, sensitive to context, and which quantify over subjective probabilities \cite{Tessler2019psychrev}. 

The meanings of many expressions in natural language are underspecified and depend on context. 
A paradigmatic example is that of gradable adjectives like \emph{tall} or \emph{expensive}.
Gradable adjectives have two defining qualities: They are vague and they are context-sensitive \cite{Kennedy2007}. 
Vagueness comes about by there not being a strict threshold beyond which the adjective is true or false (e.g., there is no single dollar amount above which a watch is expensive and below which a watch is not expensive).
Similarly, our meaning for generics (that they convey that the prevalence is greater than some threshold) is also vague: there is no single threshold above which the generic is true.
In fact, the same uncertain threshold technique that we used here is used as a way to model the vagueness of gradable adjectives \cite{Lassiter2015}.

Context-sensitivity is the second defining quality of gradable adjectives: an expensive watch lies in a completely different price range than an expensive hamburger or an expensive pack of gum.
Formally, this context-sensitivity is can be modeled as considering a comparison class, typically, of other objects of the same type \cite<e.g., an expensive hamburger is one that has a price that is high relative to other hamburgers;>{Kennedy2007}.
Our prevalence prior encodes this comparison class assumption: in our experimental tests, the prevalence prior was constructed by considering a comparison class of \emph{other animals}.
The comparison class of \emph{other animals} is sensible for generics about novel animal categories, but what can be said in general about the comparison class?
For example, why should a listener assume the comparison of \emph{other animals}, and not, say, \emph{other objects} or \emph{other entities}?
The problem of determining an appropriate comparison class is a general problem shared with other context-sensitive language, including gradable adjectives and vague quantifiers like \emph{many} or \emph{a lot} \cite{Kennedy2007, scholler2017semantic}.
Recently, it has been shown that pragmatic reasoning, formalized in a similar model of language understanding to our model of generics, can be used to flexibly adjust the comparison class for interpreting gradable adjectives depending on the context, though other factors are also likely at play \cite{Tessler2017, tessler2022warm}.
Future research should examine whether the same processes underlie comparison class inferences when interpreting generic language.

In addition to vagueness and context-sensitivity, the view of generics as a vague quantifier has a third feature that has implications for the study of generics: generics quantify over subjective, predictive probabilities.
Our view of generics is that they quantify (vaguely, context-sensitively) over probabilities or prevalences.
Though in these experiments, we did not try to discern the nature of these probabilities, in earlier work, we showed how human truth judgments are sensitive to participants subjective, predictive probabilities and not the particular ground-truth statistics concerning prevalence (Tessler \& Goodman, 2019; Expt. 2C). 
This view can help explain why certain generics sound odd or false, even though the prevalence within the appropriate comparison class seems to imply that they should be true. 
A canonical example is \emph{Primary school teachers are female}, which is a statement that conventionally is considered false (or is rejected) even though most primary school teachers are in fact female (and more primary school teachers are female in comparison to other sets of teachers, or even many other professions, etc.). 
If a speaker believes that in the future, the probability that a primary school teacher will be female is close to 0.5 (i.e., in the future, there will be no systematic gender bias in hiring), then the statement would be considered false or infelicitous under our model, similar to how it treats \emph{Birds are female} (cf., long-run frequencies in \citeNP{Cohen1999}'s theory of generics).
This fact that generics convey subjective probabilities may further be a mechanism for normative inferences from generics \cite{gockeritz2014young, roberts2017group, orvell2018s}: For example, it may be a fine line between an inference that the speaker believes the future probability \textit{is} high with the inference that the speaker believes the future probability \textit{should be} high.
Future work should examine the relationship between subjective, predictive probabilities and normative inferences from generics.
% teachers are female.

To summarize this section, our view is that quantitative, prevalence information can be at the core of generic meaning, but that such a meaning must be vague, context-sensitive, and concern subjective, predictive probabilities (see also \citeNP{Sterken2015, Nickel2016} for related ideas about context-sensitivity of generics, and see \citeNP{vanRooij2020generics} for a mathematically similar account of generics).
One prominanent feature of other views of generics that is not a part of our core theory is that of a notion of value in the core meaning of generics \cite<e.g.,>{vanRooij2020generics}.
Subjective value can enter into speaker and listener behavior in our framework (e.g., as it does in the case of polite language use; \citeNP{yoon2020polite}), but it is not a part of our core theory of the meaning of generics. 
Future studies should examine how notions of subjective value influence human inferences from generics and how best to model these effects.
%but in our three heterogenous data sets that we have modeled and presented here, we do not see any gap between the uncertain-threshold model predictions and the empirical data, suggesting that prevalence alone in the semantics of generics is sufficient.


\subsection{Prevalence priors as an intermediate representation between concepts and natural language semantics}

In our model of generics, the prevalence priors are a kind of minimal, abstract representation that lies in between rich, conceptual knowledge and natural language semantics.
We demonstrate this most clearly in Expt.~2, where we trace the effects of higher-order property knowledge observed by \citeA{Cimpian2010} back to differences in the prevalence priors.
The natural language semantic theory we examine here grounds out in the unified, quantifiable representation of beliefs as probabilities. 
Those probabilities are sampled from a prior distribution over prevalence, which we have construed as being formed by knowledge of other categories within a comparison class.
A prior distribution over prevalence, however, can be thought of more generally as a marginal distribution over prevalence given some background knowledge.

For example, taxonomic category knowledge that enables inductive generalizations across levels of a hierarchy can be learned given a grammar-based prior over category relations \cite{Kemp2008}. 
Such a probabilistic generative model reflects both conceptual knowledge through the structure of the model and statistical information with probabilities that tie together different parts of the structure.
In such a model, a prevalence prior could be constructed via marginalization over categorical or structural variables. 
With this kind of richer representation of kinds and properties (e.g., \citeNP{kemp2012integrated}), it is possible that conceptual knowledge \cite<\emph{a la}>{Gelman2003, Gelman2009}, not just prevalence, could be learned with our formal prevalence-based meaning for generics. 
Future work should examine the kinds of information other than prevalence that are communicated by generics, and how such richer conceptual models can be elaborated to account for these effects without throwing out the prevalence prior and vague threshold semantic representations.

Our computational model of generic language is compatible with representations of categories in terms both an exemplar and a prototype model \cite{nosofsky2011generalized, rosch1978principles}. A learner who infers some level of prevalence from a novel generic statement could assign that probability to all members of the category identically (\textit{a la} an exemplar approach), or could assign it specifically to prototypical members of the category and alter the imputed probability for instances that deviate from the prototype. Future experiments could try to disentangle these two alternatives. Future theoretical work additionally could try to relate these perspectives with theories of generics that are built from prototype theory directly \cite<e.g.,>{heyer1985generic}. Additionally, both exemplar and prototype representations could be imagined to play a role in the construction of the prevalence prior. For example, when a listener constructs a prevalence prior by considering the comparison class of \emph{other animals}, their representation of the comparison class (and the quantitative details of the prevalence prior) will likely vary if the class is represented by exemplars of the superordinate category or by prototypical instances of other animals. %In our experimental work, we elicit the comparison class by asking participants to list other animals, and this distribution over animals is non-uniform  

%\mht{R4: Additionally, how might prevalence be read off an exemplar/proto-type model?  If the goal is to say that relevant conceptual knowledge can be read out of prevalence priors (which was my understanding of E2's purpose), it might be worthwhile to unpack this. }

%that is not captured 

%full-blown intuitive theories could be learned via a rich

%In a probabilistic generative model of kinds and properties (e.g., \citeNP{kemp2012integrated}), one would have %Much of the psychological literature on generics has looked beyond statistical information to examine the influence of generics on conceptual knowledge. 
...
%Generics can also communicative normative beliefs about objects or people \cite{gockeritz2014young, roberts2017group, orvell2018s}. 


%  conceptual changes can occur at a higher level of abstraction above our minimalist, prevalence prior representations of world knowledge (e.g., the prevalence prior may be a lower-level variable in a rich, generative model of kinds and properties \emph{a la} \citeNP{kemp2012integrated}). 
% That is, the prevalence prior is an intermediate representation between conceptual knowledge (e.g., which could encode normative beliefs) and truth-functional natural language semantics with a quantifiable, unified metric to evaluate whether a statement is true or false. 
...
%\mht{R4: Also, if this model is intended to be a linking hypothesis it might be useful to discuss if any information might be lost by reducing a generic's interpretation to prevalence.}


\subsection{Cognitive plausibility considerations}

Our model of generic language understanding is a rational model in the mold of \citeA{Anderson1991} and \citeA{Marr1980}. 
Thus, the computations in the model are not intended to be literal cognitive processes that an individual person is calculating. 
For example, people may not explicitly represent the threshold variable and be reasoning about different thresholds when understanding generics. 
Still, the threshold formalization is useful because it makes an explicit connection to the field of formal smenatics and highlights the continuity in representation with other linguistic expressions such as vague quantifiers and gradable adjectives. 

At the same time, rational models of pragmatic reasoning have been shown to be useful models for purposes of studying individual differences \cite{franke2016reasoning, bohn2022individual, bohn2022modeling}.
Our model here is no different. For example, one could see whether inferences that are sensitive to the comparison class are also individual-specific such that if different people have different comparison classes in mind, or have different statistical knowledge of the comparison class, they will have different inferences from generics. 

%\mht{R4: it might be worthwhile to make clear earlier which assumptions are being made because formalizations require assumptions and which are the core components of the model. For example, is marginalization over a threshold intended as a cognitive process and if so is it tractable?}

%\mht{R4: Do the authors' expect the belief update works within a person? It would also be good to know if the participants in the the (a) part of the experiments were excluded from the (b) parts and were they excluded from future experiments in the paper?}


\subsection{Where does the prevalence prior come from?}

As discussed above, the prevalence prior representation we have considered in this paper is constructed by considering a comparison class of \emph{other animals} and using one's knowledge of prevalence in other categories to construct a marginal distribution on prevalence for a specific property. 
This knowledge of prevalence in other categories does not need to come from observing frequencies in the world (e.g., as done in Expt. 3).
...
In our experiments and modeling results, we make use of an implicit assumption that properties are independent by studying single generic statements about new categories. For example, in Expt. 1, a participant may learn that \emph{Glippets live in urban areas}, but this is the only fact they learned about the category \emph{glippets}. If, instead, they additionally learned that \emph{Glippets fly into building windows}, then the assumption that the properties are independent of one another might not have been able to be maintained. That is, the kind of belief updating that occurs from \emph{Glippets fly into building windows} after first learning that \emph{Glippets live in urban areas} is conceivably different than the belief updating that occurs from only learning that \emph{Glippets fly into building windows}.\footnote{We thank an anonymous reviewer for pointing out this independence assumption.} To account for non-independence, we would need to formalize more explicitly a generative model of properties and kinds (\emph{a la} \citeNP{kemp2012integrated}) and connect it to our prevalence prior-based generic intepretation model. Such a formalization is a natural next step for this line of work.


%\mht{R4: There is an independence assumption on properties in the specific cases empirically evaluated and modelled here. After hearing X is a mammal, hearing X can breathe is not the same kind of update. This seems like it would be an important part of a sufficiency claim. If the model is only meant in the specific cases here, it would be worthwhile to discuss why it wouldn't work for other important cases like non-independence. If the model is meant to be broader, it might be worthwhile to expand briefly on how this might be accommodated.}
...
\subsection{Generic language use and understanding as a rational process}
...
More broadly, speakers and listeners use and interpret generics in a manner indicative of a rational mechanism for faithful transmission of knowledge \cite{Tomasello1999}. 
...
There are some important features of our paradigm that make our experiments a kind of ``best case'' scenario for finding evidence for rational knowledge transmission and should be taken into account to understand the generalizability of this conclusion. 
...
Testing the uncertain-threshold model (and the speaker model component in \citeNP{Tessler2019psychrev}) with generics about socially sensitive topics and in development are important future directions for this approach.

\subsection{Relationship to ``generic belief'' view of generic language}

In our experiments, we predicted participants' interpretations of novel generics in terms of prevalence with our computational model as a function of empirical measurements of participants' background knowledge about properties, specifically background knowledge about the prevalence of properties. 
...
If generics are representationally primary \cite<e.g.,>{Prasada2000}, one could imagine that participants answer the prevalence prior questions (e.g., \emph{What percentage of dogs develop phobias?}) by first asking themselves internally a generic question (\emph{Do dogs develop phobias?}) and then, if the answer to this question was \emph{yes}, responding with their personal (already generically-interpreted) prevalence (and rating 0\% prevalence if the answer to the generic question was \emph{no}).
This procedure would be consistent with a view in which prevalence estimates are derived by these \enquote{generic beliefs} \cite{Prasada2000}. 
...
\subsection{Pragmatic reasoning and generic language}
