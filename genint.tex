...
\newcommand{\effsize}[2]{\(d = \rlgetnum{#1}{key}{#2}{mean}{2}\) [\rlgetnum{#1}{key}{#2}{l95}{2}, \rlgetnum{#1}{key}{#2}{u95}{2}]}
  ...
  \title{Interpreting generic statements is rationally sensitive to prior expectations}
...
We compare this model to a family of alternatives with the same background knowledge but where generics have a fixed, precise meaning; in addition, we formalize a novel quantitative model of a strictly conceptual-based approach to generics, where generics are a direct connection to conceptual knowledge.
Across three experiments in which we both measure and manipulate prior beliefs, we find the uncertain threshold approach to generics to be the best explanation of participants' highly heterogeneous interpretations of novel generic statements. 
The sufficiency of this rational model of generic interpretation suggests that generics are an effective medium for faithful transmission of knowledge between interlocutors. 
...
Understanding how beliefs are transmitted via generic language is thus an important issue for cognitive science.
...
When you encounter a dog that you have never met before, you already have expectations of the kinds of properties it may possess: it has four legs, it has a tail, it is plausible that it is friendly, it is plausible that under the right circumstances it will bark.
These expectations could be acquired through personal experiences with other dogs or they could be learned via language (e.g., hearing the [generic] statement \textit{Dogs are friendly}). 
In quantitative terms, the metric that describes beliefs about a property $f$ of an individual given the knowledge of the individual is a member of the category $k$ is the probability that an instance of a category $k$ has a feature $f$: $P(f \mid k)$ (e.g., $P(\text{X is friendly} \mid \text{X is a dog})$), sometimes referred to as the \emph{prevalence} of the feature in the category.
...
A common intuition with generics is that they \enquote{{[}\ldots{}{]} appear to be commonly taken [\emph{interpreted}] in a rather strong sense, as though the qualifier \emph{always} had implicitly crept into their interpretation} \cite<p.~172,>{Abelson1966}; in other words, a common intuition is that generics mean \emph{all}, or that the probability that a future instance will have the property $f$ is: $P(f \mid k) = 1$.
Generics allow for exceptions, however (e.g., not all dogs are friendly), and so the generic must allow for $P(f \mid k) < 1$. 
Some generics seem to convey only intermediate probabilities (i.e., levels of prevalence  substantially less than 100\%): \emph{Robins lay eggs} intuitively conveys that roughly 50\% (the female robins) lay eggs ($P(f \mid k) \approx 0.5$); \emph{Mosquitos carry malaria} should convey a low prevalence value (in true fact, $< 1\%$ of mosquitos carry malaria; $P(f \mid k) \approx 0.01$). 
...
\citeA{Tessler2019psychrev} showed that this model can explain truth judgments (or, endorsements) of generic statements (e.g., is the sentence \emph{Mosquitos carry malaria} true or false?) with a high-degree of quantitative accuracy, a fundamental problem in the study of generic language. 
...
In the first section, we describe the model of generic meaning proposed by \citeA{Tessler2019psychrev} and show how it makes predictions for prevalence interpretations from a generic statement.
We additionally describe alternative models that predict prevalence interpretations with different assumptions about the meaning of a generic; we also formalize a new model that predicts prevalence by assuming generics serve as a kind of direct conduit to conceptual knowledge.
...
We compare our uncertain threshold model to two sets of alternative models, which operate with the same prior beliefs, but which update them according to different likelihood functions. 
